...
export const VideoPlayer = ({
    ...
    // Check if device is iOS to conditionally hide controls
    ...
    const handleThumbnailClick = () => {
        if (isIOSDevice) {
            // On iOS, just hide thumbnail and let native controls take over
            setShowThumbnail(false);
            // Trigger play to start the video
            videoRef.current?.play();
        } else {
            // On other devices, use the regular toggle function
            togglePlay();
        }
    };
    ...
    return (
        ...
        >
            ...
            <div
                ...
                className={cx(
                    "group absolute inset-0 cursor-pointer rounded-[inherit] transition-all duration-300 ease-in",
                    thumbnailUrl && showThumbnail ? "visible opacity-100" : "pointer-events-none invisible opacity-0",
                    // On iOS, reduce z-index to not interfere with native controls after click
                    isIOSDevice && !showThumbnail ? "z-0" : "z-10",
                )}
            >
            ...
            <motion.video
                ...
                loop={loop}
                autoPlay={autoPlay}
                controls={isIOSDevice} // Show native controls only on iOS
                className="h-full w-full cursor-pointer rounded-[inherit] bg-black outline-1 -outline-offset-1 outline-black/10"
                onClick={isIOSDevice ? undefined : togglePlay}
                onDoubleClick={isIOSDevice ? undefined : toggleFullscreen}
            >
                <source src={src} type={type} />
                Your browser does not support the video tag.
            </motion.video>

            {/* Video Controls - Hidden on iOS */}
            {!isIOSDevice && (
                <div
                    className={cx(
                        "absolute right-0 bottom-0 left-0 translate-y-0 transform bg-linear-to-t from-black/20 to-transparent opacity-100 transition duration-150 ease-in will-change-transform",
